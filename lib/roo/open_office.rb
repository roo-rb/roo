...
module Roo
  class OpenOffice < Roo::Base
    ...
    def open_oo_file(options)
      Zip::File.open(@filename) do |zip_file|
        if (content_entry = zip_file.glob("content.xml").first)
          ...
          raise ArgumentError, ERROR_MISSING_CONTENT_XML
        end
      end
    end
    ...
    def label(labelname)
      read_labels
      return [nil, nil, nil] if @label.size < 1 || !@label.has_key?(labelname)
      [
    ...
    def find_cipher(*args)
      cipher = nil
      if args[0].eql? "http://www.w3.org/2001/04/xmlenc#aes256-cbc"
    ...
    def find_cipher_key(*args)
      if args[1].eql? "PBKDF2"
        ...
        raise ArgumentError, 'Unknown key derivation name ', args[1]
      end
      key
    end
